...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`admin`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `vi_states_base` AS select `s1`.`state` AS `state`,`s1`.`cases` AS `cases`,`s1`.`deaths` AS `deaths`,`s2`.`population` AS `population`,`s1`.`data_date` AS `data_date`,if((`s1`.`cases` = 0),0,(`s1`.`deaths` / `s1`.`cases`)) AS `death_rate` from (`vi_us_states` `s1` join `us_states` `s2`) where (`s1`.`state` = `s2`.`state`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2020-04-19 13:13:42
